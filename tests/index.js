...
describe('A11yDialog', () => {
  ...
  describe('When instantiating through the DOM, it…', () => {
    it('should instantiate the dialog without JavaScript', () => {
      const context = document.querySelector('#test-AUTO_INSTANTIATION')
      setup(context, false)
      expect(
        context.querySelector('[data-a11y-dialog]').getAttribute('aria-hidden')
      ).to.eql('true')
    })
  })
  ...
  describe('When shown and targets/siblings already have `aria-hidden` attributes, it…', () => {
    ...
    it('should save the current value of `aria-hidden` on any sibling elements to `data-a11y-dialog-original-aria-hidden` if dialog element is not natively supported', () => {
      ...
      expect(
        window.scope.alreadyhidden.hasAttribute(
          'data-a11y-dialog-original-aria-hidden'
        )
      ).to.eql(IS_DIALOG_SUPPORTED ? false : true)
    })
  })
  ...
  describe('When hidden and targets/siblings already had `aria-hidden` attributes, it…', () => {
    ...
    it('should reset `aria-hidden` of sibling element to the value of `data-a11y-dialog-original-aria-hidden` and remove the latter attribute if dialog element is not natively supported', () => {
      expect(window.scope.alreadyhidden.getAttribute('aria-hidden')).to.eql(
        'true'
      )
      expect(
        window.scope.alreadyhidden.hasAttribute(
...
function setup(scope, create = true) {
